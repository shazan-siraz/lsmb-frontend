...
import { usePackageCreateMutation } from "../../redux/features/package/packageApi";

const PackageCreate = () => {
  const { register, handleSubmit, reset } = useForm();

  const [packageCreate, {isLoading: packageCreateLoading}] = usePackageCreateMutation();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await packageCreate(packageData);
    ...
    <div>
      <div className="bg-[#EBECED] h-screen">
        <div className="text-center px-5 pt-2">
          <h1 className="font-bold text-[30px]">Make a New Package</h1>
        </div>
